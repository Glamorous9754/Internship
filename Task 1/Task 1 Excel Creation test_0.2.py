...
def create_and_save_dataframes(data1, data2, data3, data4, data5, row_count2, row_count3, row_count4, row_count5,
    ...
    # Save to Excel with previous code commented out
    # with pd.ExcelWriter(f'Combined_Tables_{current_time}.xlsx') as writer:
    #     table1.to_excel(writer, sheet_name='Approved_Action_Plan_Report', index=False)
    #     table2.to_excel(writer, sheet_name='Section_1_Plan_Summary', index=False)
    #     table3.to_excel(writer, sheet_name='Section_2_Sectoral_View', index=False)
    #     table4.to_excel(writer, sheet_name='Section_3_Scheme_View', index=False)
    #     table5.to_excel(writer, sheet_name='Section_4_Priority_Wise_Activity_Details', index=False)

    # Commented out code for saving to CSV
